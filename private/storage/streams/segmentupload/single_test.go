...
var (
	...
	fakeSegmentInfo = &streamsplit.SegmentInfo{
...
func init() {
	// This helps keep things simple and should not affect test accuracy.
	safetyMargin = 1
}
...
func TestBegin(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			upload, err := Begin(ctx, tc.beginSegment, segment, limitsExchanger, piecePutter, sched)
...
func repeatKind(kind nodeKind, count int) []nodeKind {
	var kinds []nodeKind
	for i := 0; i < count; i++ {
...
func (fakeSegment) Finalize() *streamsplit.SegmentInfo {
...
func newWrappedScheduler() *wrappedScheduler {
	return &wrappedScheduler{
		wrapped: scheduler.New(scheduler.Options{
			DurationBuffer:    0,
			InitialDuration:   0,
...
func (s *wrappedScheduler) check(expectedUploaderCount int) error {
	if len(s.handles) != 1 {
		return errs.New("expected one handle but got %d", len(s.handles))
	}
	return s.handles[0].check(expectedUploaderCount)
}
